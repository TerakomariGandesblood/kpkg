...
inline const std::string export_memory_flag =
    R"(export CFLAGS="-fPIC -fsanitize=memory -fsanitize-memory-track-origins -fsanitize-memory-use-after-dtor -fno-omit-frame-pointer -fno-optimize-sibling-calls" && export CXXFLAGS="-fPIC -fsanitize=memory -fsanitize-memory-track-origins -fsanitize-memory-use-after-dtor -fno-omit-frame-pointer -fno-optimize-sibling-calls -stdlib=libc++ -Wno-unused-command-line-argument -Wl,/usr/local/lib/libc++abi.a /usr/local/lib/libc++.a")";

inline const std::string export_thread_flag =
    R"(export CFLAGS="-fPIC -fsanitize=thread" && export CXXFLAGS="-fPIC -fsanitize=thread -stdlib=libc++ -Wno-unused-command-line-argument -Wl,/usr/local/lib/libc++abi.a /usr/local/lib/libc++.a")";
